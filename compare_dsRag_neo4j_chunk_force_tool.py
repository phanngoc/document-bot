...
# Define tools
@tool
def search_information_dsrag(query: str):
    """Retrieve information based on the query """
    print('Querying retriever:search_information_dsrag:...', query)
    search_result = dsrag_retriever.invoke(query)
    if search_result:
...
@tool
def search_information_neo4j(query: str):
    """Retrieve information based on the query """
    print('Querying retriever:search_information_neo4j:...', query)
    search_result = neo4j_retriever.invoke(query)
    if search_result:
        return "\n\n".join([doc.page_content for doc in search_result])
    else:
        return "No relevant information found."
...
react_agent_dsrag = create_react_agent(
    llm,
    tools=[search_information_dsrag],
)

react_agent_neo4j = create_react_agent(
    llm,
    tools=[search_information_neo4j],
)

def get_chain_answer(query: str):
    # Use React agents instead of direct LLM calls
    messages = [HumanMessage(content=query)]
    response_dsrag = react_agent_dsrag.invoke({"messages": messages})
    response_neo4j = react_agent_neo4j.invoke({"messages": messages})
    return response_dsrag["messages"][-1].content, response_neo4j["messages"][-1].content
...
def main():
    ...
    if user_prompt:
        response_combine = get_chain_answer(user_prompt)
        # Handle Neo4j responses
        st.session_state.messages_neo4j.append({"role": "user", "content": user_prompt})
        st.session_state.messages_neo4j.append({"role": "assistant", "content": response_combine[1]})
        
        # Handle DSRag responses
        st.session_state.messages_dsrag.append({"role": "user", "content": user_prompt})
        st.session_state.messages_dsrag.append({"role": "assistant", "content": response_combine[0]})
